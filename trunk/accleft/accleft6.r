...
alpha3 <- theta[(4 + 3 * p) + 2]
    ...
    derivlike <- matrix(derivlike, ncol = 9)
    derivlike[is.nan(derivlike)] <- 0
    #browser()
    l <- nrow(x)
    score <- cbind(derivlike[, 4: (4 +2), drop = F], diag(derivlike[, 1], l, l) %*% matrix(x, ncol = p), diag(derivlike[, 2], l, l) %*% matrix(x, ncol = p), diag(derivlike[, 3], l, l) %*% matrix(x, ncol = p), derivlike[, (4 + 3): ncol(derivlike), drop = F])
}
...
m = 25
m1 = 100
theta <- c(0.5, 0.5, 0.5, -0.5,  -1.1,   -0.2, -1.1,   -0.2, -1.1, 0.5, 0.5, 0.5)
q <- length(theta) 
mA <- matrix(NA, m, m)
mb <- matrix(NA, q, m)
n <- 621
