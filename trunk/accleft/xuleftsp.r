...
Bs3 <- as.matrix(Bs3[as.character(resp[ind3, "y2"]), ])
    ...
    pl <- ncol(mA1)
     res <- spg(c(beta1, beta2, beta3, sp1, sp2, sp3, theta), margpartial3, gr = NULL, method=2,  lower = c(rep(-100, 3 * p), rep(0, 3 * pl), 0.001), upper = c(rep(100, 3 * p), rep(10, 3 * pl), 10), project=NULL, projectArgs=NULL, control=list(M = 10, maxit = 5000), quiet=FALSE, resp, cov, n, p, cov1, cov2, cov3, Bs1, Bs2, Bs3,  A1, A2, A32, A31, ind1, ind2, ind3,  Av1, Av2)
    res
 }
...
res <-  iniestreal4(theta, bb, resp, covmy)
