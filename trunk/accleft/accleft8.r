...
cllgk.intg <- expression((log(g) +  ((log(y1)-b1x)/kappa1 ) + log( 1/y1) + log( 1/kappa1) ) *  d1 +  (log(g) + ((log(y1)-b2x)/kappa2 ) + log( 1/y1) + log(1/kappa2) ) * ((1 - d1) ) + (log(g) + ((log(y2)-b3x)/kappa3 ) +log(1/y2) + log(1/kappa3) ) * (d1) + (-  g * (exp((log(y1)-b1x)/kappa1 ) +  exp((log(y1)-b2x)/kappa2 ) + d1 * (exp((log(y2)-b3x)/kappa3 ) - exp((log(y1)-b3x)/kappa3 ))  )) )#* dbinom(x, 1, 0.8)
...
Amatx <- function(ij, vg, vq, theta,  v = 0){
    ...
    Aij <- my2d(A, mgrid, 1)#my2d1(A, cbind(XY, covm1), weight, 1)
    mA[i, j] <- Aij
    mA <<- mA
    if(i == j){
...
Amatx1 <- function(ij, vg, vq, theta,  v = 0){
    ...
    A <- function(mvt){
        vt <- mvt[, 1:2]
        x <- covm1#mvt[, 4:ncol(mvt)]
        lkgvg(vg[j], vt, x, theta) * vq[j]/ dnom * fi/dnom1
    }
    
    Aij <-my2d1(A, cbind(XY, covm1), weight, 1) #my2d(A, mgrid, 1)#
    mA[i, j] <- Aij
    mA <<- mA
    if(i == j){
        ...
        bi <- my2d1(b, cbind(XY, covm1), weight, q)#my2d(b, mgrid, q)#
